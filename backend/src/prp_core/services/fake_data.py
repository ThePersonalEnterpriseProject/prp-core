...
async def _create_transactions(account_id: uuid.UUID, num_transactions: int = 20):
    ...
    for _ in range(num_transactions):
        ...
        query = models.transactions.insert().values(
            id=uuid.uuid4(),
...
async def seed_young_professional():
    ...
    for acc in accounts_data:
        query = models.accounts.insert().values(**acc)
        await database.execute(query)
        # Generate transactions for checking and credit card
        if acc["name"] in ["Chase Checking", "Chase Sapphire"]:
            await _create_transactions(acc["id"], num_transactions=30)
...
async def seed_family():
    ...
    for acc in accounts_data:
        query = models.accounts.insert().values(**acc)
        await database.execute(query)
        if acc["name"] in ["Joint Checking", "Toyota Financial"]:
             await _create_transactions(acc["id"], num_transactions=40)
...
async def seed_small_business():
    ...
    for acc in accounts_data:
        query = models.accounts.insert().values(**acc)
        await database.execute(query)
        if acc["name"] in ["Business Checking", "Personal Checking", "Amex Business"]:
            await _create_transactions(acc["id"], num_transactions=25)
